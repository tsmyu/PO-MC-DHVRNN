...
parser.add_argument('-t_step', '--totalTimeSteps', type=int, default=596)
...
def run_sanity(args, game_files):
    ...
    for t in range(args.horizon):
        for i in range(n_agents):
            ...
            if t >= burn_in:
                ...
                losses['e_pmax'][:, t -
                                 burn_in] += batch_error(next_pos, next_pos0)
                losses['e_vel'] += batch_error(next_vel, next_vel0)
                losses['e_vmax'][:, t -
                                 burn_in] += batch_error(next_vel, next_vel0)

                losses['e_acc'] += batch_error(next_acc, next_acc0)
                losses['e_amax'][:, t -
                                 burn_in] += batch_error(next_acc, next_acc0)
                if args.in_out:
                    data[i, :, t+1, :2] = np.concatenate([next_pos], 1)
                elif args.in_sma:
                    data[i, :, t+1, n_feat*i+0:n_feat*i +
                         2] = np.concatenate([next_pos], 1)
                else:
                    data[i, :, t+1, n_feat*i+3:n_feat*i +
                         5] = np.concatenate([next_pos], 1)
    # del data
    ...
    print('Velocity (mean & best):'
          ...
          ' \pm '+'{:.2f}'.format(avgL2_sd['e_pos'])+'$ &'
          + ' $' + '{:.2f}'.format(avgL2_m['e_vel']) +
          ' \pm '+'{:.2f}'.format(avgL2_sd['e_vel'])+'$ &'
          + ' $' + '{:.2f}'.format(avgL2_m['e_acc']) +
          ' \pm '+'{:.2f}'.format(avgL2_sd['e_acc'])+'$ &'
          + ' $' + '{:.2f}'.format(avgL2_m['L_jrk']) +
          ' \pm '+'{:.2f}'.format(avgL2_sd['L_jrk'])+'$ &'
          )
    '''print('Velocity (max):'
...
def label_macro_intents(data, window_size=0):
    ...
    macro_intents_all = np.zeros(
        (N, SEQUENCE_LENGTH, N_AGENTS))  # data.shape[1]
...
def get_macro_intent(position, N_AGENTS, t):
    ...
    if N_AGENTS == 5:
        ...
        x = bound(position[0], -N_MACRO_X*MACRO_SIZE +
                  eps, N_MACRO_X*MACRO_SIZE-eps)
        y = bound(position[1], -N_MACRO_Y*MACRO_SIZE +
                  eps, N_MACRO_Y*MACRO_SIZE-eps)
...
if __name__ == '__main__':
    ...
    all_games_id = [i.split(os.sep)[-1].split('.')[0]
    ...
    init_filename0 = init_filename0 + \
        'norm/' if args.normalize else init_filename0 + 'unnorm/'
    if args.attention == 3:
    ...
    init_filename0 = init_filename0 + 'att_' + \
        str(args.attention) + '_' + str(batchSize) + '_' + str(totalTimeSteps)
    ...
    if args.cont:
        ...
        if 'MACRO' in args.model and not args.wo_macro and args.pretrain == 0:
            ...
            pretrained_dict = {k: v for k,
                               v in pretrained_list[:20] if k in model_dict}
        ...
        if args.finetune:  # args.pretrain2 == 0 and args.body:
            ...
            pretrained_dict = torch.load(
                '{}_state_dict_best.pth'.format(init_filename000))  # _pretrain2
            model_dict = model.state_dict()
            pretrained_list = list(pretrained_dict.items())
            model_dict.update(pretrained_dict)
            model.load_state_dict(model_dict)
    ...
    batchSize_test = len_seqs_test if len_seqs_test <= int(
        batchSize/2) else batchSize  # int(/4)
    ...
    if not TEST:
        train_loader = DataLoader(
            GeneralDataset(args, len_seqs_tr, train=1,
                           normalize_data=args.normalize),
            batch_size=batchSize, shuffle=False, **kwargs)
        val_loader = DataLoader(
            GeneralDataset(args, len_seqs_val, train=0,
                           normalize_data=args.normalize),
            batch_size=batchSize_val, shuffle=False, **kwargs2)
    test_loader = DataLoader(
        GeneralDataset(args, len_seqs_test, train=-1,
                       normalize_data=args.normalize),
        batch_size=batchSize_test, shuffle=False, **kwargs2)
    print('batch train: '+str(batchSize)+' val:' +
    ...
    if not TEST:
        for e in range(args.n_epoch):
            ...
            print('pretrain:'+str(hyperparams['pretrain'])+' pretrain2:'+str(
                hyperparams['pretrain2'])+' L_att:'+str(L_att))
            ...
            if e > epoch_first_best and (best_val_loss == 0 or total_val_loss < best_val_loss):
                ...
                if epoch > pretrain_time and (best_val_loss_prev-best_val_loss)/best_val_loss < 0.0001 and best_val_loss_prev != 0:
                    print('best loss - current loss: ' +
                          str(best_val_loss_prev)+' - '+str(best_val_loss))
                    break
            ...
            if epoch == pretrain_time:
                ...
                pretrained2_dict = {k: v for k,
                                    v in pretrained2_list if k in model_dict}
                model_dict.update(pretrained_dict)
                model.load_state_dict(model_dict)
    ...
    if True:
        ...
        for r in range(rep_smp):
            ...
            for batch_idx, (data, macro_intents) in enumerate(loader):
                ...
                for key in output:  # lossesが出力、keyが
                    ...
                    losses2[key][r*n_smp_b:(r+1)*n_smp_b, batch_idx*batchSize_test:(
                        batch_idx+1)*batchSize_test] = output[key].detach().cpu().numpy()
                ...
                for key in output2:
                    ...
                    losses2[key][r*n_smp_b:(r+1)*n_smp_b, batch_idx*batchSize_test:(
                        batch_idx+1)*batchSize_test] = output2[key].detach().cpu().numpy()

            for i in range(n_smp_b):
                for key in losses:
                    loss_i[r*n_smp_b+i][key] = losses[key][r *
        ...
        if True:  # create Mean + SD Tex Table for positions------------------------------------------------
            ...
            print('(mean):'
                  + ' $' +
                  '{:.2f}'.format(avgL2_m['e_pos'])+' \pm ' +
                  '{:.2f}'.format(avgL2_sd['e_pos'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                      avgL2_m['e_vel'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_vel'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                          avgL2_m['e_acc'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_acc'])+'$ &'
                  )
            print('(best):'
                  + ' $' +
                  '{:.2f}'.format(bestL2_m['e_pos'])+' \pm ' +
                  '{:.2f}'.format(bestL2_sd['e_pos'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                      bestL2_m['e_vel'])+' \pm '+'{:.2f}'.format(bestL2_sd['e_vel'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                          bestL2_m['e_acc'])+' \pm '+'{:.2f}'.format(bestL2_sd['e_acc'])+'$ &'
                  )
            print('(max):'
                  + ' $' +
                  '{:.2f}'.format(avgL2_m['e_pmax'])+' \pm ' +
                  '{:.2f}'.format(avgL2_sd['e_pmax'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                      avgL2_m['e_vmax'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_vmax'])+'$ &'
                  + ' $' +
                  '{:.2f}'.format(
                          avgL2_m['e_amax'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_amax'])+'$ &'
                  )
    ...
    if 'MACRO' in args.model and not args.wo_macro:
        ...
        for batch_idx, (data, macro_intents) in enumerate(loader):
            ...
            for i in range(n_sample):
                samples[i][:, :, batch_idx*batchSize_test:(
                    batch_idx+1)*batchSize_test] = data[:args.horizon+1, :, :, :].detach().cpu().numpy()
        ...
        for r in range(rep_smp):
            ...
            for i in range(n_smp_b):
                for key in losses:
                    loss_i[r*n_smp_b+i][key] = losses[key][r *
                                                           n_smp_b+i] / len(test_loader.dataset)
                print('CF sample '+str(r*n_smp_b+i) +
                      ':\t'+loss_str(loss_i[r*n_smp_b+i]))
            epoch_time = time.time() - start_time
            print('Time:\t {:.3f}'.format(epoch_time))  # Sample {} r*n_smp_b,
