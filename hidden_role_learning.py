...
class HiddenStructureLearning:
    ...
    def find_features_ind(self, player):
        ...
        polar_bball_ind = [n_f0+player, n_f0+player+n_pl*2,
                           n_f0+player+n_pl*4, n_f0+player+n_pl*6]  # 2-5
        polar_hoop_ind = [n_f1+player, n_f1+player+n_pl*2,
                          n_f1+player+n_pl*4, n_f1+player+n_pl*6]  # 6-9
        players_ind = list(
            range(n_f2 + player*n_pl*8, n_f2 + (player+1)*n_pl*8))  # 12-51/12-99
        ...
        if self.velocity < 2:  # not self.acc == -1:
            player_features_ind = pxy_ind + polar_bball_ind + \
                polar_hoop_ind + pvxy_ind + players_ind
        else:
            player_features_ind = pxy_ind + polar_bball_ind + \
                polar_hoop_ind + pvxy_ind + paxy_ind + players_ind

        player_features_ind2 = player_features_ind  # all

        # posHMM:
        player_features_ind = pxy_ind + \
            polar_bball_ind[:2] + polar_hoop_ind[:2]
    ...
    def find_features_ind2(self, player, reorderedPlayers):
        ...
        for i, p in enumerate(reorderedPlayers):  # i: assigned player, p: order(1:5)
            for v in range(V):  # variables
                player_features_ind[n_f0+v+p *
                                    V] = player_features_ind0[n_f0+v+i*V]
    ...
    def create_hmm_input(self, player_inds):
        ...
        player_fts = [ms[:, self.find_features_ind(
            player)[1]] for player in player_inds for ms in event]
        ...
        assert len(event[len(event)//2]
                   ) == lengths[len(lengths)//len(player_inds)//2]
        assert len(event[-1]) == lengths[-1]
        return X, lengths

    def train_hmm(self, player_inds, n_iter, random_state=42, verbose=True):
        print('Training for {0} players and {1} player_roles with iterations: {2}'.format(
            len(player_inds), self.n_roles, n_iter))
        n_pl = self.n_pl
        if n_pl == 5:
            # defend and offend players each are five
            assert len(player_inds) == n_pl
        elif n_pl == 11:
        ...
        if True:
            ...
            with open(game_files_pre+'.pkl', 'rb') as f:
                ...
                print('soccer HMM model of team ' +
                      str(self.team) + ' was loaded')
    ...
    def visualize_HMM(self, model):
        ...
        plt.savefig("figure/HMM/"+data+"_Gaussian_HMM_" +
                    XO+".png", bbox_inches='tight')
        plt.close()
    ...
    def plotDistribution(self, cmeans_mat, covars_mat, K):
        ...
        for j in range(K):
            ...
            cx = covars_mat[j, 0, 0]
            cy = covars_mat[j, 1, 1]
            e = patches.Ellipse(xy=(mx, my), width=cx,
                                height=cy, fill=False, ec=clr)
            ax.add_patch(e)
    ...
    def assign_roles(self, trainModel, player_inds, n_iter, mode='euclidean'):
        ...
        if not trainModel:
            result = self.train_hmm(
                player_inds=player_inds, n_iter=n_iter)  # train
        else:
            result = self.predict_hmm(
                trainModel, player_inds=player_inds, n_iter=n_iter)
        ...
        if self.meanHMM:
            ...
            for i in range(n_seq):
                ed[start:start+lengths[i],
                    :] = np.mean(ed[start:start+lengths[i], :], axis=0)
                start += lengths[i]
        ...
        # it should be divisibe by number of players
        assert len(ed) % len(player_inds) == 0
        # n = len(ed)

        # unnecessary to be corrected when n_pl != n_roles
        role_assignments = np.zeros((n, len(player_inds)), dtype=np.int)
        for i in range(n):
            # n_roles*n_roles  row i is assigned to column j.
            cost = ed[np.arange(len(player_inds))*n + i]
        ...
        # role_assignments: (seqs,players), result['X']: (seqs*players)*features
    ...
    def reorder_moment(self):
        ...
        if not self.Model_d:  # train
            print('Total HMM training took {0:.2f}mins'.format(
                (time.time()-t1)/60))
            ...
            for i in range(len(lengths)):
                ...
                for j in range(lengths[i]):
                    ...
                    for k, p in enumerate(players):
                        ...
                        # n_pl==N_roles(Le+17)
                        if ra_i.shape[1] == self.n_roles:
                            re_p_ind = self.find_features_ind2(
                                ra_i[j][k]+offset, ra_i[j])[0]
                        reordered[i][j][p_ind] = original[i][j][re_p_ind]
            return reordered
        reordered_defend = copy.deepcopy(reorder_moment_(
            self.defend_players, original, reordered, defend_role_assignments, 0))
        reordered_all = copy.deepcopy(reorder_moment_(
            self.offend_players, original, reordered_defend, offend_role_assignments, self.n_pl))
    ...
    def visualize(self, original, reordered_all):
        ...
        for n1 in range(0, 5, 1):
            ...
            plt.savefig("figure/HMM/{}_{}.png".format(data, n1),
                        bbox_inches='tight')
            plt.close()
    ...
    def draw_trajectory3(self, samples, f_ax, allPlayers):
        ...
        for i in range(0, timestep-2, 1):
            self.plotPosition3(team_A, team_B, ball, K, i,
                               markersize, TextCircle, allPlayers)
        ...
        if K == 5:
            ...
            markersize = 0.5
            TextCircle = 2
        self.plotPosition3(team_A, team_B, ball, K, index,
                           markersize, TextCircle, allPlayers)
    ...
    def plotPosition3(self, team_A, team_B, ball, K, i, markersize, TextCircle, allPlayers):
        ...
        if allPlayers:
            if TextCircle == 1:
                im_ball = patches.Circle(
                    (ball[i, 0], ball[i, 1]), radius=markersize-0.15, fc='orange', ec="k")
                ax.add_patch(im_ball)
                ax.text(ball[i, 0], ball[i, 1], 'B',
                        color='w', ha='center', va='center')
            elif TextCircle == 0:
                im_ball = patches.Circle(
                    (ball[i, 0], ball[i, 1]), radius=0.1, fc='orange', ec="k")
                ax.add_patch(im_ball)
                im_ball = plt.scatter(
                    ball[i, 0], ball[i, 1], marker=".", s=markersize, fc='orange', ec="k")  # for legend only
        ...
        for tm in range(Tm):
            ...
            for j in range(start_pl, n_team_agent):
                ...
                if TextCircle == 1:  # player circle
                    ...
                    # player jersey # (text)
                    ax.text(xx, yy, str(j+1), color='w',
                            ha='center', va='center')
                elif TextCircle == 2:
                    ax.text(xx, yy, str(j+1), color='k',
                            ha='center', va='center')

                else:
                    im_team[tm] = plt.scatter(
                        xx, yy, marker=".", s=markersize, ec=clr, color=clr)
