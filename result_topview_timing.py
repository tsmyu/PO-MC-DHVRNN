...
path = r"C:\Users\yota-\OneDrive - 同志社大学\PO-MC-DHVRNN\result\20251015"
...
####### kiku #######
    ...
    #         # Measured path arrow
    #         x_train = train_x_pre[i]
    #         y_train = train_y_pre[i]
    ...
    #         ax.arrow(x_train, y_train, dx, dy, head_width=0.1, head_length=0.1, fc='b', ec='b',zorder=3)
    ...
    #         dx_test = np.cos(angle_rad_test) * 0.2
    #         dy_test = np.sin(angle_rad_test) * 0.2
    #         ax.arrow(x_test, y_test, dx_test, dy_test, head_width=0.1, head_length=0.1, fc='r', ec='r',zorder=3)
            
    #         angle_diff = abs(angle_deg - angle_test)
    #         angle_diffs.append(angle_diff)
    #         # print(f"count: {count}, i: {i}, angle_deg: {angle_deg:.2f}, angle_test: {angle_test:.2f}, diff: {angle_diff:.2f}")

    for i in range(len(train_pulse)):
        if train_pulse[i] >= 0.5:
            ax.scatter(train_x_pre[i], train_y_pre[i], label='measured pulse timing', color='k', s=20, zorder=4)
        if test_pulse[i] >= 0.555:
    ...
    #     print(f"count: {count}, Mean Abs Diff: {mean_diff:.2f}, Std Dev: {std_diff:.2f}")
