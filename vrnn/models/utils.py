...
def nll_gauss(mean, std, x, pow=False, Sum=True):
    ...
    if not pow:
        ...
        nll_element = (
            (x - mean).pow(2) / std + torch.log(std) + torch.log(2 * pi)
        )

    nll = (
        0.5 * torch.sum(nll_element) if Sum else 0.5 * torch.sum(nll_element, 1)
    )
    return nll
...
def kld_gauss(mean_1, std_1, mean_2, std_2, Sum=True):
    ...
    kld = (
        0.5 * torch.sum(kld_element) if Sum else 0.5 * torch.sum(kld_element, 1)
    )
    return kld
...
def calc_dist_cos_sin(rolePos, refPos, batchSize):
    ...
    rolefeat[loc1, 1] = (rolePos[loc1, 0] - refPos[loc1, 0]) / rolefeat[
        loc1, 0
    ]  # cos
    rolefeat[loc1, 2] = (rolePos[loc1, 1] - refPos[loc1, 1]) / rolefeat[
        loc1, 0
    ]  # sin
...
def roll_out(
    ...
    prev_feature = y_t
    next_feature = y_t1i
    next_vel_std_feature = torch.zeros(
        next_feature.size(0),
        2,
        dtype=next_feature.dtype,
        device=next_feature.device,
    )
    ...
    # egacy_next = next_current.reshape(batchSize, n_all_agents, n_feat)
    new_matrix = torch.zeros((batchSize, n_all_agents, n_feat)).to(device)
    std_matrix = torch.zeros((batchSize, n_all_agents, 2)).to(device)
    ...
    vel_std_long = next_vel_std_feature.to(device)
    ...
    std_matrix[:, roleOrder, :] = vel_std_long
    ...
    new_std_vector = torch.reshape(std_matrix, (batchSize, n_all_agents * 2))
    # import pdb; pdb.set_trace()

    return new_feature_vector, new_std_vector
