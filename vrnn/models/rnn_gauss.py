...
class RNN_GAUSS(nn.Module):
    ...
    def __init__(self, params, parser=None):
        ...
        self.dec_mean = nn.ModuleList(
            [nn.Linear(h_dim, x_dim - 1) for i in range(n_agents)]
        )
        self.dec_std = nn.ModuleList(
            [
                nn.Sequential(nn.Linear(h_dim, x_dim - 1), nn.Softplus())
                for i in range(n_agents)
            ]
        )
    ...
    def sample(
        ...
        out2["e_pmax"] = (
            ...
            if len_time == burn_in
            else torch.zeros(n_sample, batchSize, len_time - burn_in).to(device)
        )
        out2["e_vmax"] = (
            torch.zeros(n_sample, batchSize, len_time).to(device)
            if len_time == burn_in
                    ...
                    if t >= burn_in:  # and not CF_pred:
                        ...
                        if burn_in == len_time:
                            ...
                            out2["e_vmax"][n, :, t - burn_in] += batch_error(
                                v_t1, v0_t1, Sum=False
                            )
                            out2["e_amax"][n, :, t - burn_in] += batch_error(
                                a_t1, a0_t1, Sum=False
                            )
