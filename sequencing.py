...
def subsample_sequence(events, subsample_factor, random_sample=False):
    ...
    def subsample_sequence_(moments, subsample_factor, random_sample=False):
        ...
        if subsample_factor > 0:
            n_intervals = (
                moments_len // subsample_factor
            )  # number of subsampling intervals
        else:
        ...
        if random_sample:
            if left != 0:
                rs = [
                    np.random.randint(0, subsample_factor)
                    for _ in range(n_intervals)
                ] + [np.random.randint(0, left)]
            else:
                rs = [
                    np.random.randint(0, subsample_factor)
                    for _ in range(n_intervals)
            ...
            if (
                round(subsample_factor * 10) == round(subsample_factor) * 10
            ):  # int
...
def get_bat_sequence_data(sequence_data, in_sma):
    ...
    fix_data = np.zeros(
        (
            sequence_data.shape[0],
            sequence_data.shape[1],
            sequence_data.shape[2] - 1,
            sequence_data.shape[3],
        )
    )
    if in_sma:
        # 2次元データ用
        ...
        if n_pl == 5:
            for pl in range(npl):
                if not in_sma:
                    ...
                    index = np.append(
                        index, index0[116 + pl + p * npl * 4]
                    )  # cos 1
                    index = np.append(
                        index, index0[126 + pl + p * npl * 4]
                    )  # sin 2
                index = np.append(
                    index, index0[pl * 2 : pl * 2 + 2]
                )  # positions 3-4
                if velocity >= 0:
                    # velocities 5-6
                    index = np.append(
                        index, index0[506 + pl * 2 : 506 + pl * 2 + 2]
                    )
                if velocity == 2:
                    index = np.append(
                        index, index0[529 + pl * 2 : 529 + pl * 2 + 2]
                    )  # acceleration
                if not in_sma:
            ...
            # k nearest players
            if (
                k_nearest > 0 and k_nearest < 10
            ):  # players regardless of attackers and defenders
                index = np.append(
                    index, np.zeros(n_feat * k_nearest)
                )  # temporary
            ...
            for pl in range(npl):
                if not in_sma:
                    ...
                    index = np.append(
                        index, index0[244 + pl + p * npl * 4]
                    )  # cos 1
                    index = np.append(
                        index, index0[266 + pl + p * npl * 4]
                    )  # sin 2
                index = np.append(
                    index, index0[pl * 2 : pl * 2 + 2]
                )  # positions 3-4
                if velocity >= 0:
                    # velocities 5-6
                    index = np.append(
                        index, index0[2158 + pl * 2 : 2158 + pl * 2 + 2]
                    )
                if velocity == 2:
                    # velocities 5-6
                    index = np.append(
                        index, index0[2204 + pl * 2 : 2204 + pl * 2 + 2]
                    )
                if not in_sma:
                    # relation with the goal 7-9 (th is not used)
                    index = np.append(
                        index, index0[134 + pl : 134 + npl * 3 + pl - 1 : npl]
                    )
                    # relation with the ball 10-12
                    index = np.append(
                        index, index0[46 + pl : 46 + npl * 3 + pl - 1 : npl]
                    )
            # k nearest players
            if (
                k_nearest > 0 and k_nearest < 10
        ...
        for i in single_game:
            ...
            for t in range(i_len):
                ...
                if (
                    ...
                    ind_nearest = ind_nearest[np.nonzero(ind_nearest)][
                        :k_nearest
                    ]
                    for k in range(k_nearest):
                        index[
                            ...
                            ind_nearest[k] * n_feat : ind_nearest[k] * n_feat
                            + n_feat
                        ]
            ...
            if i_len >= sequence_length:
                sequences0 = [
                    sequence0[-sequence_length:, :]
                    if j + sequence_length > i_len - 1
                    else sequence0[j : j + sequence_length, :]
                    for j in range(
                        0, i_len - overlap, sequence_length - overlap
                    )
                ]  # for the states
                ...
                if velocity == 2:
                    action = [
                        np.roll(
                            kk[:, p * n_feat + 3 : p * n_feat + 9], -1, axis=0
                        )[:-1, :]
                        for kk in sequences0
                    ]
                    # action2 = [np.roll(kk[:, p*2:p*2+2], -1, axis=0)[:-1, :] for kk in sequences]
                elif velocity == 1:
                    action = [
                        np.roll(
                            kk[:, p * n_feat + 3 : p * n_feat + 7], -1, axis=0
                        )[:-1, :]
                        for kk in sequences0
                    ]
                elif velocity:
                    ...
                    action = [
                        np.roll(
                            kk[:, p * n_feat + 3 : p * n_feat + 5], -1, axis=0
                        )[:-1, :]
                        for kk in sequences0
                    ]
