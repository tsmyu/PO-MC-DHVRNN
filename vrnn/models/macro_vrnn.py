...
class MACRO_VRNN(nn.Module):
    def __init__(self, params, parser=None):
        ...
        self.body_pretrain = (
            params["pretrain2"] > 0
        )  # two-state learning  (currently not considered)
    ...
    def forward(
        ...
        if hp["pretrain"]:
            ...
            if self.pred_type == 0:
                ...
                out2["L_pulse_flag"] = torch.zeros(1).to(device)
                out["L_pulse_freq"] = torch.zeros(1).to(device)
            elif self.pred_type == 1:
                out["L_rec"] = torch.zeros(1).to(device)
            elif self.pred_type == 2:
                out2["L_pulse_flag"] = torch.zeros(1).to(device)
        ...
        pulse_freq_m_arr = torch.zeros(batchSize).to(device)
        pulse_freq_p_arr = torch.zeros(batchSize).to(device)
            ...
            if not hp["pretrain"]:
                ...
                for i in range(n_agents):
                    ...
                    if self.pred_type == 0:
                        ...
                        pulse_freq_p_arr += torch.t(
                            torch.where(dec_pulse_t >= 0.5, 1, 0)
                        )[0]
        ...
        if hp["pretrain"]:
            ...
            if self.pred_type == 0:
                ...
                out2["L_pulse_flag"] /= (len_time) * n_agents
                out["L_pulse_freq"] = torch.sum(
                    torch.sqrt((pulse_freq_m_arr - pulse_freq_p_arr).pow(2))
                )
            elif self.pred_type == 1:
                out["L_kl"] /= (len_time) * n_agents
                out["L_rec"] /= (len_time) * n_agents
    ...
    def sample(
        ...
        soft_att = (
            []
        )  # torch.zeros(len_time,n_agents,batchSize, n_all_agents+1, n_sample)
        hard_att = torch.zeros(
            len_time,
            n_agents,
            batchSize,
            n_all_agents + 1,
        ...
        if burn_in == len_time:
            ...
            if not TEST:  # validation
                out2["e_pmax"] = torch.sum(
                    torch.max(
                        out2["e_pmax"] / n_agents,
                        dim=2,
                    )[0]
                )
                out2["e_vmax"] = torch.sum(
                    torch.max(
                        out2["e_vmax"] / n_agents,
                        dim=2,
                    )[0]
                )
                out2["e_amax"] = torch.sum(
                    torch.max(
                        out2["e_amax"] / n_agents,
                        dim=2,
                    )[0]
                )
            else:
                out2["e_pmax"] = torch.max(
                    out2["e_pmax"] / n_agents,
                    dim=2,
                )[0]
                out2["e_vmax"] = torch.max(
                    out2["e_vmax"] / n_agents,
                    dim=2,
                )[0]
                out2["e_amax"] = torch.max(
                    out2["e_amax"] / n_agents,
                    dim=2,
                )[0]
