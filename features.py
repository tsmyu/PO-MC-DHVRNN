...
def flatten_moments(events_df, normalize):
    ...
    def flatten_moment(moment, normalize):
        ...
        features = np.concatenate((m[1:11, 2:4].reshape(-1)*feet_m,    # x,y of all 10 players
                                   ...
                                   # quarter number
                                   np.array([moment[0]]),
                                   # time in seconds left to the end of the period
                                   np.array([moment[2]]),
        ...
        if normalize:
            ...
            features[:SEQUENCE_DIMENSION] = np.divide(
                features[:SEQUENCE_DIMENSION]-SHIFT, NORMALIZE)
    ...
    df['flattened'] = df.moments.apply(
        lambda ms: [flatten_moment(m, normalize) for m in ms])
    # just use the first one to determine
    df['team_ids'] = df.moments.apply(lambda ms: get_team_ids(ms[0]))
...
def create_dynamic_features(events_df, fs, n_pl, velocity):
    ...
    def create_dynamic_features_(moments, fs, n_pl, velocity):
        ...
        # get the players x,y and basketball x,y,z coordinates
        pxy = moments[:, :n_pl*4+ball_dim]
        next_pxy = np.roll(pxy, -1, axis=0)  # get next frame value
        # the last velocity is not meaningful
        vel = ((next_pxy - pxy)/fs)[:-1, :]
        ...
        return out
    df['enriched'] = df.moments.apply(
        lambda ms: create_dynamic_features_(ms, fs, n_pl, velocity))
    return df['enriched'].values
...
class OneHotEncoding:
    ...
    def __init__(self, cat=None):
        ...
        # temporarily just one hot encode two teams
