...
class PlotGame:
    ...
    def load_moment2img(
        self, data, event_number, moment_number, return_img=False
    ):
        """load_moment2img
        ...
        ball_xy = np.array(
            [x[5][0][2:5] for x in data["events"][ii]["moments"]]
        )
        ...
        for kk in range(player.shape[0]):
            ...
            # player circle
            player_circ = plt.Circle(
                (xx, yy), 2.2, facecolor=color_dict[team_id][0], edgecolor="k"
            )
        ...
        plt.plot(
            team_1_xy_mean[: jj + 1, 0],
            team_1_xy_mean[: jj + 1, 1],
            "o",
        ...
        plt.plot(
            team_2_xy_mean[: jj + 1, 0],
            team_2_xy_mean[: jj + 1, 1],
            "o",
...
def plot_check(game_data, plt_ind=0):
    ...
    assert plt_ind < len(
        game_data
    ), "The plotting index is larger than the length of the game."
...
def split_baseon_data(target_array):
    ...
    for env_num in ENV_nums:
        ...
        print(f"ENV:{env_num}")
        print(f"total data number = {n_total}")
        print(f"train data number = {n_train}")
        print(f"validation data number = {n_val}")
        print(f"test data number = {n_total - n_train_val}")

        train_idx_list.append(indices[:n_train])
        val_idx_list.append(indices[n_train : n_train + n_val])
        test_idx_list.append(indices[n_train_val:])
