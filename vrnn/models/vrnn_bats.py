...
class VRNN_bats(nn.Module):

    def __init__(self, params, mid_units_for_enc_prior, mid_units_for_dec, gru_units, gru_micro_num, optimizer, dropout_num):
        super().__init__()
        x_dim = params['x_dim']
        y_dim = params["y_dim"]
        in_state0 = x_dim 
        ...
        dropout = dropout_num
        self.n_layers = 2
        self.mid_units_for_enc_prior = mid_units_for_enc_prior
        self.mid_units_for_dec = mid_units_for_dec
        self.gru_units = gru_units
        self.gru_micro_num = gru_micro_num
        self.optimizer = optimizer
        
        self.batchnorm = True  
        if self.batchnorm:
            self.bn_enc = nn.ModuleList(nn.BatchNorm1d(mid_units_for_enc_prior)) 
            self.bn_prior = nn.ModuleList(nn.BatchNorm1d(mid_units_for_enc_prior))   
            self.bn_dec = nn.ModuleList(nn.BatchNorm1d(mid_units_for_dec)) 
         
        in_prior = in_state0 + in_state+self.mid_units_for_enc_prior+self.gru_micro_num 
        in_enc = in_prior + rnn_in_x
        in_enc_pre = in_prior + x_dim       

        self.enc = nn.ModuleList(nn.Sequential(
            nn.Linear(in_enc, self.mid_units_for_enc_prior),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(self.mid_units_for_enc_prior, self.mid_units_for_enc_prior),
            nn.ReLU(),
            nn.Dropout(dropout)))

        self.enc_mean = nn.ModuleList(nn.Linear(self.mid_units_for_enc_prior, self.gru_units))
        self.enc_std = nn.ModuleList(nn.Sequential(
            nn.Linear(self.mid_units_for_enc_prior, self.gru_units),
            nn.Softplus()))

        self.prior = nn.ModuleList(nn.Sequential(
            nn.Linear(in_prior, self.mid_units_for_enc_prior), # m_dim+rnn_micro_dim
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(self.mid_units_for_enc_prior, self.mid_units_for_enc_prior),
            nn.ReLU(),
            nn.Dropout(dropout)))
        self.prior_mean = nn.ModuleList(nn.Linear(self.mid_units_for_enc_prior, self.gru_units))
        self.prior_std = nn.ModuleList(nn.Sequential(
            nn.Linear(self.mid_units_for_enc_prior, self.gru_units),
            nn.Softplus()))

        self.dec = nn.ModuleList(nn.Sequential(
            nn.Linear(in_state0 + in_state+self.mid_units_for_dec+self.gru_units+self.gru_micro_num, self.mid_units_for_dec), # y_dim
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(self.mid_units_for_dec, self.mid_units_for_dec),
            nn.ReLU(),
            nn.Dropout(dropout)))
        self.dec_mean = nn.ModuleList(nn.Linear(self.mid_units_for_dec, rnn_in_x-1))
        

        #if not self.fixedsigma:
        self.dec_std = nn.ModuleList(nn.Sequential(
            nn.Linear(self.mid_units_for_dec, rnn_in_x-1),
            nn.Softplus()))
        # self.dec_pulse = nn.ModuleList([nn.Sequential(
        #     nn.Linear(h_dim, 1),
        #     nn.Sigmoid()) for i in range(n_agents)])
        self.dec_pulse = nn.ModuleList(nn.Sequential(
            nn.Linear(self.mid_units_for_dec, 1)))    

        self.gru_micro = nn.ModuleList(nn.GRU(rnn_in_x+self.gru_units, self.gru_micro_num, self.n_layers))
    
    def forward(self, states, rollout):
        ...
        h_macro = torch.zeros(self.n_layers, batchSize, 0)
        ...
        for t in range(len_time):
            ...
            if self.in_sma:  # 2dim
                ...
                next_pulse = (
                    states[t + 1][0][:,5].clone().reshape(-1, 1)
                )
            ...
            if self.attention == -1: # w/o embedding and attention
                state_in = y_t
            else:
                print("wrong")
                exit()
            prior_in = torch.cat([state_in0, state_in, m_t[0], h_micro[0][-1]], 1)
            enc_in = torch.cat([x_t0_with_pulse, prior_in], 1)
            enc_t = self.enc[0](enc_in)

            if self.batchnorm:
                enc_t = self.bn_enc(enc_t)
            enc_mean_t = self.enc_mean(enc_t)
            enc_std_t = self.enc_std(enc_t)

            prior_t = self.prior(prior_in)
            if self.batchnorm:
                prior_t = self.bn_prior(prior_t)
            prior_mean_t = self.prior_mean(prior_t)
            prior_std_t = self.prior_std(prior_t)

            z_t = sample_gauss(enc_mean_t, enc_std_t)

            dec_t = self.dec(torch.cat([state_in0,state_in, m_t, z_t, h_micro[-1]], 1))
            if self.batchnorm:
                dec_t = self.bn_dec(dec_t)

            dec_mean_t = self.dec_mean(dec_t)
            dec_std_t = self.dec_std(dec_t)
            dec_pulse_t = self.dec_pulse(dec_t)

            _, h_micro = self.gru_micro(torch.cat([x_t0_with_pulse, z_t], 1).unsqueeze(0), h_micro)
