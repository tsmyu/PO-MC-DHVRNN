...
def index_by_agent(states, n_agents):
    ...
    x = x.view(
        x.size(0), x.size(1), n_agents, -1
    ).transpose(1, 2)
    return x


def get_macro_ohe(macro, n_agents, M):
    macro_ohe = torch.zeros(
        macro.size(0), n_agents, macro.size(1), M
    )
    for i in range(n_agents):
        macro_ohe[:, i, :, :] = one_hot_encode(
            macro[:, :, i].data, M
        )
    if macro.is_cuda:
...
def nll_gauss(mean, std, x, pow=False, Sum=True):
    ...
    nll = (
        0.5 * torch.sum(nll_element)
        if Sum
        else 0.5 * torch.sum(nll_element, 1)
    )
    return nll


def kld_gauss(
    mean_1, std_1, mean_2, std_2, Sum=True
):
    kld_element = (
        2 * torch.log(std_2)
        - 2 * torch.log(std_1)
    ...
    kld = (
        0.5 * torch.sum(kld_element)
        if Sum
        else 0.5 * torch.sum(kld_element, 1)
    )
    return kld
...
def entropy_gauss(std, scale=1):
    ...
    pi, e = torch.FloatTensor(
        [math.pi]
    ), torch.FloatTensor([math.e])
    if std.is_cuda:
        pi, e = pi.cuda(), e.cuda()
    return 0.5 * torch.sum(
        scale * torch.log(2 * pi * e * std)
    )


def batch_error(
    predict, true, Sum=True, sqrt=True, diff=True
):
...
def acc_cost(
    next_vel, current_abs_vel, fs, acc_bound
):
    # ReLu-like acceleration cost function
    next_abs_vel = torch.sqrt(
        torch.sum((next_vel).pow(2), 1)
    )
    diff_vel = next_abs_vel - (
        current_abs_vel + acc_bound * fs
    )
    loc = (diff_vel > 0).nonzero()
    return torch.sum(diff_vel[loc])
...
def calc_dist_cos_sin(rolePos, refPos, batchSize):
    ...
    rolefeat[:, 0] = torch.sqrt(
        (rolePos[:, 0] - refPos[:, 0]).pow(2)
        + (rolePos[:, 1] - refPos[:, 1]).pow(2)
    )  # dist
    loc0 = (rolefeat[:, 0] == 0).nonzero()
    loc1 = (rolefeat[:, 0] != 0).nonzero()

    rolefeat[loc1, 1] = (
        rolePos[loc1, 0] - refPos[loc1, 0]
    ) / rolefeat[
        loc1, 0
    ]  # cos
    rolefeat[loc1, 2] = (
        rolePos[loc1, 1] - refPos[loc1, 1]
    ) / rolefeat[
        loc1, 0
    ]  # sin
...
def roll_out(
    ...
    if acc == 0:  # vel
        next_vel_pulse = prediction_all[
            :, :, :dim_x
        ]
        dim_vel_pulse = dim_x
        dim_vel = dim_vel_pulse - 1
    elif (
        acc == 1 or acc == -1 or acc == 3
    ):  # pos,vel,(acc)
        error(
            "not modified yet. dim should be defined"
        )  # TBD
    ...
    next_current = next_feature[
        :, 0:n_feat_player
    ]

    legacy_next = next_current.reshape(
    ...
    roleOrderList = [
        role for role in range(n_roles)
    ]
    role_long = torch.zeros(
        (batchSize, n_feat)
    ).to(device)
    ...
    if acc >= 0 and acc < 4:
        ...
        role_long[
            :, dim_vel : dim_vel * 2
        ] = next_vel_pulse[:, roleOrder, :dim_vel]
        role_long[
            :, dim_vel_pulse * 2
        ] = next_vel_pulse[
    ...
    if acc == 1 or acc == 3 or acc == -1:
        ...
        for idx, prev_f in enumerate(
            prev_feature
        ):
            next_point = role_long[idx][:dim_vel]
            prev_point = prev_prev_faeture[idx][
                :dim_vel
            ]
            # [X, Y, Vx, Vy, θ, pulse_flag, Env, Bat, state]
            if prev_f[5] >= 0.5:
    ...
    for teammate in teammateList:
        ...
        if (
            ...
            teamRoleInd = roleOrderList.index(
                teammate
            )

            if acc >= 2:  # vel,acc
                player[:, dim * 2 :] = next_acc[
                    :, teamRoleInd, :
                ]
