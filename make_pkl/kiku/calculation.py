import numpy as np
import json
# from obstacle_information.wall import wall_x, wall_y
...
def calc_obs_area(obs_point_dict):
    ...
    for env in obs_point_dict.keys():
        if env == "wall":
            ...
            obs_frame_x, obs_frame_y = calc_each_point(obs_point_dict[env])
...
obs_point_dict = json.load(
    open("./obstacle_information/Envs.json", "r")
)
...
def vertical_angle(x, y, z):
    ...
    angle = []
    for i in range(len(x) - 2):
        vec1 = [x[i + 1] - x[i], y[i + 1] - y[i], z[i + 1] - z[i]]
        vec2 = [x[i + 2] - x[i + 1], y[i + 2] - y[i + 1], z[i + 2] - z[i + 1]]
        absvec1 = np.linalg.norm(vec1)
        absvec2 = np.linalg.norm(vec2)
        inner = np.dot(vec1, vec2)
        cos_theta = inner / (absvec1 * absvec2)
...
def cross_point(x: list, y: list, rot_x: list, rot_y: list, env_name: str):
    """
    交点までの距離を算出
    """
